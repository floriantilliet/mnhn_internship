...
preds['predTATA']=np.concatenate((np.zeros(250),model2.predict(X_chr4_TATA,batch_size=256).ravel(),np.zeros(250)))
preds['pred100']=np.concatenate((np.zeros(250),model2.predict(X_chr4_100,batch_size=256).ravel(),np.zeros(250)))

os.chdir('/home/florian/projet/drosoSYNTH/')
np.savez_compressed('preds_'+model_name+"_synth",**preds)
